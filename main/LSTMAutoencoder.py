...
class LSTMAutoencoder(object):
    ...
    def __init__(self, step_num, hidden_num, inputs,
        ...
        with tf.variable_scope('decoder') as vs:
            ...
            if decode_without_input:  # input with zeros sequences
                ...
                """
                if reverse:
                    # list reversed time_step*[batch_size,hidden_num]
                ...
                self.output_ = tf.matmul(tf.reshape(dec_output_, [-1, hidden_num]),
                                         dec_weight_) + dec_bias_
                self.output_ = tf.reshape(self.output_, [-1, step_num, self.elem_num])
